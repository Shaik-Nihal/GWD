...
function animateStats() {
  document.querySelectorAll('.stat-number[data-animate]').forEach(el => {
    const target = parseInt(el.getAttribute('data-animate'), 10);
    let current = 0;
    const duration = 1400;
    ...
    const counter = setInterval(() => {
      ...
      el.textContent = current + (target === 100 && el.nextElementSibling?.textContent === "Alumni Satisfaction" ? "%" : "");
    }, stepTime);
  });
}
window.addEventListener('DOMContentLoaded', animateStats);

// Overview card flip (keyboard accessible)
document.querySelectorAll('.overview-card-flip').forEach(card => {
  card.tabIndex = 0;
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.faq-question').forEach(btn => {
    btn.addEventListener('click', function () {
      document.querySelectorAll('.faq-question').forEach(b => b !== btn && b.classList.remove('active'));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', e => {
      ...
      if (input.value) {
        alert("Thank you for subscribing to Apollo's newsletter!");
        input.value = '';
      }
    });
  }
...
// ...retain all existing carousel, menu, modal, and smooth scroll JS...
...
// --- JOURNEY TIMELINE INTERACTIVE SCRIPT ---
document.addEventListener('DOMContentLoaded', function () {
  ...
  const label = document.querySelector('.timeline-step-label .timeline-step-index');
  ...
  function updateTimeline(idx) {
    ...
    // Progress bar
    progress.style.width = `${(idx) / (cards.length - 1) * 100}%`;
...
// ...rest of your JS from previous version...

// Training module script

document.addEventListener('DOMContentLoaded', function () {
    const modules = document.querySelectorAll('.modules-carousel .module-card');
    ...
    function previousModule() {
        let prev = (current - 1 + modules.length) % modules.length;
        showModule(prev);
        resetInterval();
    }
    ...
    function resetInterval() {
        ...
        intervalId = setInterval(nextModule, 3000);
    }

    // Button events
    if (nextBtn) nextBtn.addEventListener('click', nextModule);
    if (prevBtn) prevBtn.addEventListener('click', previousModule);

    // Indicator events
    indicators.forEach((ind, i) => {
        ind.addEventListener('click', () => goToModule(i));
    });

    // Start auto-advance
    showModule(0);
    intervalId = setInterval(nextModule, 5000);
});